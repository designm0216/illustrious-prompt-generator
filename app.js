...
function updateNegativePrompt() {
    ...
    const hasNSFWRating = Array.from(appState.global).some(tag => tag.includes('questionable') || tag.includes('explicit'));
    const hasNSFWTags = Array.from(appState.global).some(tag => {
        const tagObj = findTagByValue(tag);
        return tagObj && tagObj.nsfw;
    });
    const hasCharacterNSFW = appState.characters.some(char => Object.values(char).some(set => Array.from(set).some(tag => {
        const tagObj = findTagByValue(tag);
        return tagObj && tagObj.nsfw;
    })));
    ...
    const scanDatabase = (obj) => {
        Object.values(obj).forEach(item => {
            if (Array.isArray(item)) {
                item.forEach(tag => {
                    if (tag && tag.value && tag.label) tagTranslationMap.set(tag.value, tag.label);
                });
...
function updateTranslationDisplay() {
    ...
    if (appState.global.size > 0) {
        ...
        if (globalTags.length > 0) sections.push({ title: '品質・スタイル・設定', tags: globalTags, icon: '⭐' });
    }
    ...
    appState.characters.forEach((character, index) => {
        ...
        if (charTags.length > 0) sections.push({ title: `キャラクター ${index + 1}`, tags: charTags, icon: '👤' });
    });

    const cameraBgTags = [...Array.from(appState.camera), ...Array.from(appState.background)].map(translateTag);
    if (cameraBgTags.length > 0) sections.push({ title: 'カメラ・背景', tags: cameraBgTags, icon: '📷' });

    if (appState.lighting.size > 0) {
        const lightingTags = Array.from(appState.lighting).map(translateTag);
        sections.push({ title: '照明・仕上げ', tags: lightingTags, icon: '💡' });
    }
...
function findTagByValue(value) {
    const searchInObject = (obj, results = []) => {
        Object.values(obj).forEach(val => {
            ...
            else if (val && typeof val === 'object' && !val.id) searchInObject(val, results);
        });
        return results;
    };
...
function savePreset() {
    ...
    const presetData = {
        ...
        characters: appState.characters.map(char => {
            const charData = {};
            Object.keys(char).forEach(key => charData[key] = Array.from(char[key]));
            return charData;
        }),
...
function loadPreset() {
    ...
    appState.characters = data.characters.map(char => {
        const newChar = {};
        Object.keys(char).forEach(key => newChar[key] = new Set(char[key]));
        return newChar;
    });
